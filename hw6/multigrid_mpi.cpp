...
void compute_residual(double *u, double *rhs, double *res, int N, double invhsq, int rank = 0)
{
  int i;
  for (i = 1; i < N-1; i++)
    res[i*N] = (rhs[i*N] - (2.*u[i] - u[i-1] - u[i+1]) * invhsq);
}
...
int main(int argc, char * argv[])
{
  ...
  for (l = 0; l < levels; ++l) {
    ...
    Nfull[l] = Nfine / (int) pow(2,l); //Add two for ghost points
    double h = 1.0 / (Nfull[l]);
    hsq[l] = h * h;
    printf("MG level %2d, N = %8d, Rank = %d\n", l, N[l],world_rank);
    invhsq[l] = 1.0 / hsq[l];
    u[l]    = (double *) calloc(N[l]+1,sizeof(double));
    for (int i=0; i<N[l]+1; ++i) {
      u[l][i]=0.;
    }
    rhs[l] = (double *) malloc(sizeof(double) * (Nfull[l]+1)); //Subtract 1 due to ghost point?
  }
  /* rhs on finest mesh */
  for (i = 0; i <= Nfull[0]; ++i) {
    rhs[0][i] = 1.0;
  ...
  compute_residual(u[0], rhs[0], res, N[0], invhsq[0], world_rank);
  res_norm = res0_norm = compute_norm(res, N[0]);
  printf("Initial Residual: %f\n", res0_norm); 

  if(world_rank == 0){
    for(int i = 0; i < N[0]; i++){
      printf("u[0][%d] = %f on process %d\n", i,u[0][i],world_rank);
    }
  }
