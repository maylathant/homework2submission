...
double compute_norm(double *u, int N)
{
  ...
  double norm = 0.0;
  #ifdef _OPENMP
  #pragma omp parallel for private(i), shared(u,N), reduction(+:norm)
  #endif
  for (i = 0; i <= N; i++)
    norm += u[i] * u[i];
  return sqrt(norm);
...
void coarsen(double *uf, double *uc, int N) {
  int ic;
  #ifdef _OPENMP
  #pragma omp parallel for private(ic), shared(N,uc,uf)
  #endif
  for (ic = 1; ic < N/2; ++ic)
...
void refine_and_add(double *u, double *uf, int N)
{
  ...
  uf[1] += 0.5 * (u[0] + u[1]);
  #ifdef _OPENMP
  #pragma omp parallel for private(i), shared(N,uf,u)
  #endif
  for (i = 1; i < N; ++i) {
    uf[2*i] += u[i];
...
void compute_residual(double *u, double *rhs, double *res, int N, double invhsq)
{
  int i;
  #ifdef _OPENMP
  #pragma omp parallel for private(i), shared(u,invhsq,rhs,N)
  #endif
  for (i = 1; i < N; i++)
...
void jacobi(double *u, double *rhs, int N, double hsq, int ssteps)
{
  ...
  for (j = 0; j < ssteps; ++j) {
    #ifdef _OPENMP
    #pragma omp parallel for private(i,utemp), shared(u,omega,hsq,rhs)
    #endif
    for (i = 1; i < N; i++){
      utemp = u[i];
...
int main(int argc, char * argv[])
{
  ...
  if (argc < 3 || argc > 5) {
    ...
    fprintf(stderr, "num_threads: # of openmp threads\n");
    abort();
  }
